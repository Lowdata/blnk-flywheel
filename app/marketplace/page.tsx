"use client";
/**
 * /marketplace — the BLNK Holder Hub, ported from the standalone Vite app.
 *
 * Wallet, chain and query providers come from the root layout; this route adds
 * the BLNK adapter on top. Per CLAUDE.md, everything here reads wallet state,
 * so the whole tree is mount-gated rather than branching to a different JSX
 * shape on the server pass.
 */
import { useMounted } from '@/hooks/useMounted';
import { BlnkProvider } from '@/marketplace/contracts/provider';
import '@/marketplace/hub.css';
...
import { PALETTE } from "@/marketplace/theme/palette";
import { CHAIN_NAME } from "@/marketplace/chain/robinhood";
import { useWallet } from "@/marketplace/hooks/useWallet";
import { useBalls, useOwnedNfts, useWhitelist } from "@/marketplace/hooks/useHolderData";
import { useStakeActions } from "@/marketplace/hooks/useStakeActions";
import { BALLS_PER_DAY, isEarning } from "@/marketplace/lib/balls";
import { NftCard } from "@/marketplace/components/NftCard";
import { MarketCard, type MarketItem } from "@/marketplace/components/MarketCard";
import { MarketDetail } from "@/marketplace/components/MarketDetail";
import { ColorBallsPanel } from "@/marketplace/components/ColorBallsPanel";
import { ConnectButton } from "@/marketplace/components/ConnectButton";
import { StateFrame, SkeletonGrid } from "@/marketplace/components/StateFrame";
import { TxToast } from "@/marketplace/components/TxToast";
import { GhostButton, GradientButton, Pill, Wordmark } from "@/marketplace/components/Primitives";
import { WhitelistMarketplace } from "@/marketplace/components/WhitelistMarketplace";
import type { BlnkNft } from "@/marketplace/contracts/types";
...
function HolderHub() {
  ...
  // Only the tokens in flight should show a spinner — `busy` alone would light
  // up all 200 cards while a single stake confirms.
  ...
  return (
    <div className="h-screen w-full flex flex-col overflow-hidden blnk-grid" style={{ backgroundColor: PALETTE.bg }}>
      ...
      <main className="flex-1 flex flex-col min-h-0 max-w-[1600px] w-full mx-auto px-5 py-4">
        {/* Title row */}
        ...
        {/* Main 3-column layout, fixed height, no page scroll */}
        <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-[1.35fr_1fr_0.85fr] gap-4">
          {/* Left: My NFTs */}
          ...
          <section className="flex flex-col min-h-0">
            ...
            <div className="flex-1 min-h-0">
              <ColorBallsPanel
                ...
                // Only "loading" while a wallet is actually being read. Without
                // the address check, the disconnected state renders an empty
                // void instead of the "still grey" prompt.
...
function NftPane({
  ...
  return (
    <div className="h-full min-h-0 overflow-y-auto blnk-scroll pr-1.5">
      <div className="grid grid-cols-3 gap-3 auto-rows-[minmax(200px,1fr)] min-h-full">
        {nfts.map((nft) => (
          <NftCard
            key={nft.tokenId}
            nft={nft}
