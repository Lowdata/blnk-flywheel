...
export const devState: DevState = process.env.NODE_ENV !== "production"
  ? ((params?.get("state") as DevState) ?? null)
  : null;

export const devMockCount: number | undefined = process.env.NODE_ENV !== "production"
  ? Number(params?.get("mock")) || undefined
  : undefined;

const rawWallet = process.env.NODE_ENV !== "production" ? (params?.get("wallet") ?? "") : "";
